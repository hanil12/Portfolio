...
HIT_RESULT CircleCollider::Block(shared_ptr<CircleCollider> other)
{
    ...
    if (this->IsCollision(other))
    {
        ...
        Vector2 temp = other->GetTransform()->GetPos() + dir.NormalVector2() * overlap;
        other->GetTransform()->SetPosition(temp);
        other->GetTransform()->UpdateSRT();
...
HIT_RESULT CircleCollider::Block(shared_ptr<RectCollider> other)
{
    ...
    if (other->IsAABB(shared_from_this()))
    {
        ...
        if (circlePos.x > leftTop.x && circlePos.x < rightTop.x)
        {
            ...
            Vector2 temp = other->GetTransform()->GetPos() + dir * (sum - distance);
            other->GetTransform()->SetPosition(temp);
            other->GetTransform()->UpdateSRT();
        ...
        else
        {
            ...
            dir.Normalize();
            Vector2 temp = other->GetTransform()->GetPos() + dir * magnitude;
            other->GetTransform()->SetPosition(temp);
